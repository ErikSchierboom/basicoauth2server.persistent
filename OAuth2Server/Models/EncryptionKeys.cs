namespace OAuth2Server.Models
{
    ...
    /// <summary>
    /// For demo purposes, we use default keys. In a real application, you would read this data from certificate files
    /// stored on your local machine. One can clearly see that we have separate keys for the resource server and the 
    /// authorization server. For each of those keys, we have a public and private key. The use of this becomes clear
    /// when we look at both the authorization server (which outputs the tokens) and the resource server (which takes
    /// tokens). If we do encryption with a public key on the authorization server, we need to do decryption with a
    /// private key on the resource server. The same goes for the signging key. This means that the <see cref="ResourceServerEncryptionPublicKey"/>
    /// and <see cref="AuthorizationServerSigningPrivateKey"/> keys are only used by the resource server, whereas
    /// the <see cref="ResourceServerEncryptionPrivateKey"/> and <see cref="AuthorizationServerSigningPublicKey"/> keys
    /// are only used by the resource server. 
    /// </summary>
    public static class EncryptionKeys
    {
        ...
        /// <summary>
        /// The default resource server private key used for encrypting access tokens. In a real-life situation, you would
        /// get this from a certificate file.
        /// </summary>
        private static readonly RSAParameters ResourceServerEncryptionPrivateKey = new RSAParameters
                                                                                       {
        ...
        /// <summary>
        /// The authorization server signing private key, which is used for private signing operations. In a real-life situation, you would
        /// get this from a certificate file.
        /// </summary>
        private static readonly RSAParameters AuthorizationServerSigningPrivateKey = new RSAParameters
                                                                                         {
                                                                                             Exponent = new byte[] { 1, 0, 1 }, 
        ...
        /// <summary>
        /// The authorization server signing public key, which is used for public signing operations. In a real-life situation, you would
        /// get this from a certificate file.
        /// </summary>
        private static readonly RSAParameters AuthorizationServerSigningPublicKey = new RSAParameters
                                                                                        {
                                                                                            Exponent = new byte[] { 1, 0, 1 },
                                                                                            Modulus = new byte[] { 210, 95, 53, 12, 203, 114, 150, 23, 23, 88, 4, 200, 47, 219, 73, 54, 146, 253, 126, 121, 105, 91, 118, 217, 182, 167, 140, 6, 67, 112, 97, 183, 66, 112, 245, 103, 136, 222, 205, 28, 196, 45, 6, 223, 192, 76, 56, 180, 90, 120, 144, 19, 31, 193, 37, 129, 186, 214, 36, 53, 204, 53, 108, 133, 112, 17, 133, 244, 3, 12, 230, 29, 243, 51, 79, 253, 10, 111, 185, 23, 74, 230, 99, 94, 78, 49, 209, 39, 95, 213, 248, 212, 22, 4, 222, 145, 77, 190, 136, 230, 134, 70, 228, 241, 194, 216, 163, 234, 52, 1, 64, 181, 139, 128, 90, 255, 214, 60, 168, 233, 254, 110, 31, 102, 58, 67, 201, 33 },
                                                                                        };
        ...
        /// <summary>
        /// Gets the resource server encryption public key.
        /// </summary>
        ...
        /// <summary>
        /// Gets the resource server encryption private key.
        /// </summary>
        /// <returns>The crypto service provider.</returns>
        ...
        /// <summary>
        /// Gets the authorization server signing private key.
        /// </summary>
        /// <returns>The crypto service provider.</returns>
        ...
        /// <summary>
        /// Gets the authorization server signing public key.
        /// </summary>
        /// <returns>The crypto service provider.</returns>
